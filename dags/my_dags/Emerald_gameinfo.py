...
def _summoner_task(ti):
    summonerNames = SummonerName.requeset_summonerNames(SummonerAPI.EMERALD)
    ti.xcom_push(key="summonersEmerald", value = summonerNames)

def _puuid_task(ti):
    puuid_instance = Puuid()
    summonerNames = ti.xcom_pull(key="summonersEmerald")
    puuids = puuid_instance.collect_puuids(summonerNames)
    ti.xcom_push(key="puuidsEmerald", value=puuids)

def _matchID_task(ti):
    matchID_instance = MatchID()
    matchIDs = matchID_instance.collect_matchIDs(ti.xcom_pull(key="puuidsEmerald"))
    ti.xcom_push(key="matchIDsEmerald", value=matchIDs)

def _gameinfo_task(ti):
    gameinfo_instance = Gameinfo()
    gameinfo_instance.collect_gameinfo(key_name="EMERALD",
                                       matchIDs=ti.xcom_pull(key="matchIDsEmerald"))


with DAG("emerald_gameinfo", start_date=datetime(2024, 1, 1),
