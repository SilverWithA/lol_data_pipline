...
def _summoner_task(ti):
    summonerNames = SummonerName.requeset_summonerNames(SummonerAPI.MASTER)
    ti.xcom_push(key="summonersMaster", value = summonerNames)

def _puuid_task(ti):
    puuid_instance = Puuid()
    summonerNames = ti.xcom_pull(key="summonersMaster")
    puuids = puuid_instance.collect_puuids(summonerNames)
    ti.xcom_push(key="puuidsMaster", value=puuids)

def _matchID_task(ti):
    matchID_instance = MatchID()
    matchIDs = matchID_instance.collect_matchIDs(ti.xcom_pull(key="puuidsMaster"))
    ti.xcom_push(key="matchIDsMaster", value=matchIDs)

def _gameinfo_task(ti):
    gameinfo_instance = Gameinfo()
    gameinfo_instance.collect_gameinfo(key_name="MASTER",
                                       matchIDs=ti.xcom_pull(key="matchIDsMaster"))


with DAG("master_gameinfo", start_date=datetime(2024, 1, 1),
