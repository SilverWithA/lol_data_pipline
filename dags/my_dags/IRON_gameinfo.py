...
def _summoner_task(ti):
    summonerNames = SummonerName.requeset_summonerNames(SummonerAPI.IRON)
    ti.xcom_push(key="summonersIRON", value = summonerNames)

def _puuid_task(ti):
    puuid_instance = Puuid()
    summonerNames = ti.xcom_pull(key="summonersIRON")
    puuids = puuid_instance.collect_puuids(summonerNames)
    ti.xcom_push(key="puuidsIRON", value=puuids)

def _matchID_task(ti):
    matchID_instance = MatchID()
    matchIDs = matchID_instance.collect_matchIDs(ti.xcom_pull(key="puuidsIRON"))
    ti.xcom_push(key="matchIDsIRON", value=matchIDs)

def _gameinfo_task(ti):
    gameinfo_instance = Gameinfo()
    gameinfo_instance.collect_gameinfo(key_name="IRON",
                                       matchIDs=ti.xcom_pull(key="matchIDsIRON"))


with DAG("IRON_gameinfo", start_date=datetime(2024, 1, 1),
