...
def _summoner_task(ti):
    summonerNames = SummonerName.requeset_summonerNames(SummonerAPI.GOLD)
    ti.xcom_push(key="summonersGOLD", value = summonerNames)

def _puuid_task(ti):
    puuid_instance = Puuid()
    summonerNames = ti.xcom_pull(key="summonersGOLD")
    puuids = puuid_instance.collect_puuids(summonerNames)
    ti.xcom_push(key="puuidsGOLD", value=puuids)

def _matchID_task(ti):
    matchID_instance = MatchID()
    matchIDs = matchID_instance.collect_matchIDs(ti.xcom_pull(key="puuidsGOLD"))
    ti.xcom_push(key="matchIDsGOLD", value=matchIDs)

def _gameinfo_task(ti):
    gameinfo_instance = Gameinfo()
    gameinfo_instance.collect_gameinfo(key_name="GOLD",
                                       matchIDs=ti.xcom_pull(key="matchIDsGOLD"))


with DAG("GOLD_gameinfo", start_date=datetime(2024, 1, 1),
