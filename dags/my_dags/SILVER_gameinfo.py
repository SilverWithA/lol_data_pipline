...
def _summoner_task(ti):
    summonerNames = SummonerName.requeset_summonerNames(SummonerAPI.SILVER)
    ti.xcom_push(key="summonersSILVER", value = summonerNames)

def _puuid_task(ti):
    puuid_instance = Puuid()
    summonerNames = ti.xcom_pull(key="summonersSILVER")
    puuids = puuid_instance.collect_puuids(summonerNames)
    ti.xcom_push(key="puuidsSILVER", value=puuids)

def _matchID_task(ti):
    matchID_instance = MatchID()
    matchIDs = matchID_instance.collect_matchIDs(ti.xcom_pull(key="puuidsSILVER"))
    ti.xcom_push(key="matchIDsSILVER", value=matchIDs)

def _gameinfo_task(ti):
    gameinfo_instance = Gameinfo()
    gameinfo_instance.collect_gameinfo(key_name="SILVER",
                                       matchIDs=ti.xcom_pull(key="matchIDsSILVER"))


with DAG("SILVER_gameinfo", start_date=datetime(2024, 1, 1),
