...
def _summoner_task(ti):
    summonerNames = SummonerName.requeset_summonerNames(SummonerAPI.BRONZE)
    ti.xcom_push(key="summonersBRONZE", value = summonerNames)

def _puuid_task(ti):
    puuid_instance = Puuid()
    summonerNames = ti.xcom_pull(key="summonersBRONZE")
    puuids = puuid_instance.collect_puuids(summonerNames)
    ti.xcom_push(key="puuidsBRONZE", value=puuids)

def _matchID_task(ti):
    matchID_instance = MatchID()
    matchIDs = matchID_instance.collect_matchIDs(ti.xcom_pull(key="puuidsBRONZE"))
    ti.xcom_push(key="matchIDsBRONZE", value=matchIDs)

def _gameinfo_task(ti):
    gameinfo_instance = Gameinfo()
    gameinfo_instance.collect_gameinfo(key_name="BRONZE",
                                       matchIDs=ti.xcom_pull(key="matchIDsBRONZE"))


with DAG("BRONZE_gameinfo", start_date=datetime(2024, 1, 1),
